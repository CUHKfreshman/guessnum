...
contract RandomMatchmaking {
    ...
    function joinGame(uint256 seed) public {
        ...
        // 检查玩家是否已经进入房间
        require(!isPlayerInRoom(msg.sender), "Already in the Room");
    ...
    // 检查玩家是否已经在房间
    function isPlayerInRoom(address player) public view returns (bool) {
    ...
    function getPlayerRoomNumber(address player1) public view returns (uint256, address player2, uint256,
        ...
        for (uint256 i = 1; i < nextRoomNumber; i++) {
            Room storage room = rooms[i];
            if (room.player1 == player1 && room.isRoomActive) {
